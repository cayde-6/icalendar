...
export const buildTimeRange = (start?: string, end?: string): TimeRange | undefined => {
  ...
  const startDate = start ? new Date(start) : endDate ? addDays(endDate, -30) : new Date()
  const resolvedEndDate = endDate ?? addDays(startDate, 30)

  if (Number.isNaN(startDate.getTime()) || Number.isNaN(resolvedEndDate.getTime())) {
  ...
  return {
    ...
    end: toCalDavTime(resolvedEndDate),
  }
}
